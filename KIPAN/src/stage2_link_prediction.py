...
def load_stage1_outputs(log: RunLogger):
    ...
    train_sample_ids = splits_data["splits"][primary_seed]["train"]

    # by gd["common_samples"]. Convert string IDs -> integer positional indices.
    sample_id_to_pos = {sid: i for i, sid in enumerate(gd["common_samples"])}
    train_patient_idx = np.array(
        [sample_id_to_pos[s] for s in train_sample_ids if s in sample_id_to_pos],
        dtype=np.int64
    )
...
def fit_gmm_importance(centrality_df: pd.DataFrame, log: RunLogger):
    ...
    all_high_genes  = set(centrality_df[centrality_df["importance_label"] == "High"]["name"])
    all_data_genes  = set(centrality_df["name"])
    mrna_nodes      = centrality_df[centrality_df["omic_type"] == "mRNA"]
    mrna_high_genes = set(mrna_nodes[mrna_nodes["importance_label"] == "High"]["name"])

    cgc_in_data       = C.COSMIC_CGC_GENES & all_data_genes
    cgc_recovered     = C.COSMIC_CGC_GENES & all_high_genes
    cgc_in_mrna       = C.COSMIC_CGC_GENES & set(mrna_nodes["name"])
    cgc_recovered_mrna= C.COSMIC_CGC_GENES & mrna_high_genes
    recovery_rate     = len(cgc_recovered) / max(len(cgc_in_data), 1)

    kipan_genes_set  = set().union(*C.KIPAN_GENE_SETS.values())
    pam50_in_data    = kipan_genes_set & all_data_genes
    pam50_recovered  = kipan_genes_set & all_high_genes
    ...
    log.log_metric("CGC_genes_in_mRNA",        len(cgc_in_mrna), fmt="d")
    log.log_metric("CGC_recovered_mRNA",       len(cgc_recovered_mrna), fmt="d")
    log.log_metric("KIPAN_genes_in_dataset",   len(pam50_in_data), fmt="d")
    log.log_metric("KIPAN_genes_recovered",    len(pam50_recovered), fmt="d")
    log.log_metric("KIPAN_recovery_rate",      pam50_rate)
    ...
    gmm_data = {
        ...
        "validation": {
            ...
            "cgc_in_mrna":         len(cgc_in_mrna),
            "cgc_recovered_mrna":  len(cgc_recovered_mrna),
            "kipan_in_data":       len(pam50_in_data),
            "kipan_recovered":     len(pam50_recovered),
            "kipan_rate":          round(pam50_rate, 4),
        },
    }
...
def rank_biomarkers(centrality_df: pd.DataFrame,
    ...
    df["in_kirc_signature"] = df["name"].isin(C.KIPAN_GENE_SETS)
...
def generate_stage2_figures(centrality_df: pd.DataFrame,
    ...
    ax.set_title(f"Top {len(high_df)} High-Importance KIPAN Biomarkers (HOBIT)")
...
def main():
    ...
    log.section("HOBIT-KIPAN  Stage 2 — Link Prediction & GMM Importance")
